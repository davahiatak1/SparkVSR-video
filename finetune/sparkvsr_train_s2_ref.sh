...
# Model Configuration
MODEL_ARGS=(
    --model_path "checkpoints/sparkvsr-s1"
...
OUTPUT_ARGS=(
    ...
    --output_dir "checkpoints/sparkvsr-s2"
    --report_to "wandb"
    --tracker_name "sparkvsr-s2"
)
...
DATA_ARGS=(
    ...
    --train_resolution "2x320x640"  # (frames x height x width)
    --image_ratio 0.5
)
...
CHECKPOINT_ARGS=(
    ...
    # --resume_from_checkpoint "/data1/tzz/JZYu/DOVE_checkpoints/DOVE-s2-ref-i2v/checkpoint-500"  # if you want to resume from a checkpoint, otherwise, comment this line
)
...
/data1/tzz/anaconda_dir/envs/DOVE/bin/accelerate launch --config_file accelerate_config.yaml train.py \
    "${MODEL_ARGS[@]}" \
    "${LORA_ARGS[@]}" \
    "${OUTPUT_ARGS[@]}" \
    "${DATA_ARGS[@]}" \
